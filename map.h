...
public:
    ...
    map(const key_compare& cmp,
        ...
        comparer(cmp), allocator(alloc), item_count(0) { }
    ...
    map(const map& r) :
        comparer(r.comparer), allocator(r.allocator)
    {
...
template <class K, class D, class C, class Alloc>
inline void swap(map<K, D, C, Alloc>& a,
                 map<K, D, C, Alloc>& b)
{
    a.swap(b);
}
